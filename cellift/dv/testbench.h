...
#endif // VM_TRACE
      if ((module_->data_mem_req_o & 1) == 1 && (module_->data_mem_we_o & 1) == 1) {
          if ((module_->data_mem_addr_o & 0x7FFFFFFFULL) == 0x60000000ULL) {
            if (ret.type != REQ_NONE)
              std::cerr << "WARNING: Missed a request during a batch of ticks." << std::endl;
            ret.type = REQ_STOP;
          } else if ((module_->data_mem_addr_o & 0x7FFFFFFFULL) == 0x60000010ULL) {
            if (ret.type != REQ_NONE)
              std::cerr << "WARNING: Missed a request during a batch of ticks." << std::endl;
            // assert(module_->data_mem_strb_o == 0xFF);
            ret.type = REQ_INTREGDUMP;
            ret.content = module_->data_mem_wdata_o;
          } else if ((module_->data_mem_addr_o & 0x7FFFFFFFULL) == 0x60000018ULL) {
            if (ret.type != REQ_NONE)
              std::cerr << "WARNING: Missed a request during a batch of ticks." << std::endl;
            // assert(module_->data_mem_strb_o == 0xFF);
